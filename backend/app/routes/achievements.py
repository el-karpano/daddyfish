...
ACHIEVEMENTS = [
    {"id": "first_trip", "title": "Первый заброс", "description": "Совершить первую рыбалку", "icon": "🎣"},
    ...
    {"id": "big_catch", "title": "Богатый улов", "description": "Поймать 20 рыб за одну рыбалку", "icon": "🐟"},
    ...
    {"id": "water_conqueror", "title": "Покоритель вод", "description": "Посетить 10 разных мест", "icon": "🗺"},
    ...
    {"id": "sharpshooter", "title": "Знаток рыб", "description": "Поймать 5 разных видов рыбы", "icon": "🎯"},
    ...
    {"id": "record_breaker", "title": "Новый рекорд", "description": "Побить собственный рекорд по весу рыбы", "icon": "🏆"},
...
def get_achievements(user: dict = Depends(get_club_member)):
    user_id = user["id"]
...
@router.get("/achievements/{target_user_id}")
def get_user_achievements(target_user_id: int, user: dict = Depends(get_club_member)):
    """Get achievements for another club member."""
    # Verify target is in same club
    club = user["_club"]
    ...
    for ach in ACHIEVEMENTS:
        calc = _calc_achievement(ach, stats, records_data, all_catches, photo_records)
        calc["unlocked"] = calc["progress"] >= calc["target"]
        calc["newly_unlocked"] = ach["id"] in unlocked_ids and calc["unlocked"]
        results.append(calc)
