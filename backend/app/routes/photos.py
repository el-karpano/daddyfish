...
@router.get("")
def list_photos(record_id: int, user: dict = Depends(get_club_member)):
    resp = supabase.table("fishing_records").select("id").eq("id", record_id).execute()
    if not resp.data:
...
@router.post("")
async def upload_photo(record_id: int, file: UploadFile = File(...), user: dict = Depends(get_club_member)):
...
def delete_photo(record_id: int, photo_id: int, user: dict = Depends(get_club_member)):
    resp = supabase.table("fishing_records").select("id").eq(
        "id", record_id
    ).eq("user_id", user["id"]).execute()
    if not resp.data:
